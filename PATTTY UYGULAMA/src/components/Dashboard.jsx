...
import { Plus, Flame, Check, Smile, Zap, Moon, Thermometer, Heart, ChevronRight, MapPin, Store, ScanLine } from 'lucide-react';
...
const Dashboard = ({ setPet, openAddModal, onOpenVetLocator, onOpenFoodScan }) => {
    const { t } = useLanguage();
    ...
    const VetLocatorBanner = () => (
        <div 
            onClick={onOpenVetLocator}
            className="group relative overflow-hidden rounded-[2rem] bg-gradient-to-r from-emerald-500 to-teal-600 p-1 cursor-pointer shadow-xl shadow-teal-500/20 active:scale-[0.98] transition-all flex-1 min-w-[45%]"
        >
            ...
            <div className="relative bg-white/10 backdrop-blur-md rounded-[1.8rem] p-4 h-full flex flex-col justify-between border border-white/20">
                <div className="w-12 h-12 rounded-2xl bg-white/20 flex items-center justify-center text-white backdrop-blur-md shadow-inner border border-white/10 mb-3">
                    <MapPin size={24} />
                </div>
                <div className="text-white">
                    <h3 className="font-bold text-base leading-tight">{t('dash_find_places')}</h3>
                    <p className="text-[10px] text-teal-50 font-medium mt-1 opacity-90 line-clamp-1">{t('dash_find_places_desc')}</p>
                </div>
    ...
    const FoodScanBanner = () => (
        <div 
            onClick={onOpenFoodScan}
            className="group relative overflow-hidden rounded-[2rem] bg-gradient-to-r from-orange-400 to-rose-500 p-1 cursor-pointer shadow-xl shadow-orange-500/20 active:scale-[0.98] transition-all flex-1 min-w-[45%]"
        >
            ...
            <div className="relative bg-white/10 backdrop-blur-md rounded-[1.8rem] p-4 h-full flex flex-col justify-between border border-white/20">
                ...
                <div className="text-white">
                    <h3 className="font-bold text-base leading-tight">{t('food_scan_title')}</h3>
                    <p className="text-[10px] text-orange-50 font-medium mt-1 opacity-90 line-clamp-1">{t('scan_instruction')}</p>
                </div>
        ...
        <div className="h-full flex flex-col items-center justify-center text-center p-8 space-y-8 relative overflow-hidden animate-in fade-in">
            ...
            <div className="flex flex-col gap-4 w-full max-w-xs z-10">
                ...
                <div className="grid grid-cols-2 gap-3 w-full">
                    <button onClick={onOpenVetLocator} className="flex flex-col items-center justify-center gap-2 bg-white dark:bg-white/10 text-gray-700 dark:text-white px-4 py-3 rounded-2xl font-bold border border-gray-200 dark:border-white/10 active:scale-95 transition-all text-xs">
                        <MapPin size={20}/> {t('nav_vet')}
                    </button>
                    <button onClick={onOpenFoodScan} className="flex flex-col items-center justify-center gap-2 bg-white dark:bg-white/10 text-gray-700 dark:text-white px-4 py-3 rounded-2xl font-bold border border-gray-200 dark:border-white/10 active:scale-95 transition-all text-xs">
                ...
                <div className="relative bg-white/5 backdrop-blur-sm rounded-[2.3rem] p-6">
                    <div className="flex justify-between items-start mb-6">
                        <div>
                            <h3 className="text-2xl font-bold mb-1 transition-all flex items-center gap-2">
                                ...
                                {hasLoggedToday && <span className="bg-white/20 text-[10px] px-2 py-0.5 rounded-full">Güncellendi</span>}
                            </h3>
                    ...
                    {hasLoggedToday ? (
                        <div className="bg-white/20 backdrop-blur-md rounded-2xl p-4 flex items-center gap-4 border border-white/10 animate-in zoom-in duration-300">
                            ...
                            <div>
                                <p className="font-bold text-lg leading-tight">{t('great_job') || "Harika İş!"}</p>
                                <p className="text-xs opacity-80 mt-0.5">Bugünün kaydı tamamlandı.</p>
                            </div>
                        </div>
                    ) : (
                        <div>
