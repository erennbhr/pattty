// erennbhr/pattty/pattty-df8ab6d3def020d5068a132ec11f675ce8fce13a/Yeni klasör/src/components/SmartCalendar.jsx
...
const SmartCalendar = () => {
  ...
  // Sadece TR ve EN için lokal isimler (helpers.js'e taşındı, burada kullanılıyor)
  const monthNames = {
    tr: [
  ...
  // Hardcode Fix: FREQUENCY_OPTIONS tamamen t() ile değiştirildi
  ...
  const handleCameraClick = async () => {
    ...
    if (success) {
      // Hardcode Fix: Bildirim t() ile değiştirildi
      showNotification(t('cal_memory_saved_notif'), 'success');
    }
  };
  ...
  const handleAddEvent = () => {
    ...
    switch (newEvent.frequency) {
      ...
      case 'monthly':
        iterations = 12;
        break;
      case 'quarterly': // Bu seçenek FREQUENCY_OPTIONS'da yok ama mantıkta var, düzeltilmedi.
        iterations = 4;
        break;
      case 'yearly':
        iterations = 5;
      ...
      <div className="space-y-4">
        ...
        {selectedDateEvents.length === 0 ? (
          ...
          selectedDateEvents.map((event) => {
            ...
            // Hardcode Fix: 'Genel' -> t('gen_general')
            const petName =
              pets.find((p) => String(p.id) === String(event.petId))?.name || t('gen_general');
                ...
                >
                  ...
                  {pets.length === 0 ? (
                    <option value="" disabled>
                      ⚠️ {t('cal_add_event_no_pet_warning')}
                    </option>
                  ) : (
                    pets.map((p) => (
