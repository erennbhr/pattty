// erennbhr/pattty/pattty-df8ab6d3def020d5068a132ec11f675ce8fce13a/Yeni klasör/src/components/WeightChart.jsx
...
const WeightChart = ({ pet, setPets }) => {
    ...
    const addWeight = () => {
        ...
        // BUG FIX: finalWeight her zaman KG olarak kaydedilmeli. Display unit'e göre değişmemeli.
        const finalWeight = totalKg; 
    ...
    // Ağırlıklar her zaman KG olarak kaydedildiğinden, görüntüleme sırasında dönüşüm yapılır.
    const displayWeights = weights.map(w => ({ 
        ...w, 
        // 1 KG = 2.20462 LBS
        val: unit === 'kg' ? w.weight : w.weight * 2.20462 
    }));
        ...
        <div className="space-y-4 animate-in slide-in-from-bottom duration-500">
            ...
            <div className="flex gap-2 items-center">
                <div className="flex-1 flex gap-2">
                    <input type="number" min="0" placeholder={t('weight_input_kg')} value={kg} onChange={e=>setKg(e.target.value)} className="w-full p-3 rounded-xl border-none bg-gray-100 dark:bg-neutral-800 text-center dark:text-white font-bold outline-none focus:ring-2 focus:ring-indigo-500/20"/>
                    <span className="self-center font-bold text-gray-400">.</span>
                    <input type="number" min="0" placeholder={t('weight_input_gr')} value={gr} onChange={e=>setGr(e.target.value)} className="w-full p-3 rounded-xl border-none bg-gray-100 dark:bg-neutral-800 text-center dark:text-white font-bold outline-none focus:ring-2 focus:ring-indigo-500/20"/>
                </div>
