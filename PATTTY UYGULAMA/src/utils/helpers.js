...
export const adjustColor = (color, amount) => {
  ...
  // Hatalı olan regex/substr mantığı yerine padStart kullanıyoruz:
  const result = hex.match(/.{2}/g).map(c => {
    const val = parseInt(c, 16);
    // Renk değerini artır/azalt ve 0-255 arasına sıkıştır
    const adjusted = Math.min(255, Math.max(0, val + amount));
    // Hex'e çevir ve başına gerekirse 0 ekle (padStart)
...
export const TRANSLATIONS = {
  tr: {
    ...
    // YENİ EKLEMELER: YARDIM, GİZLİLİK, KOŞULLAR
