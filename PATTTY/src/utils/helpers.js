...
import { app } from './firebase'; // Firebase config dosyanızın yolu (src/utils/firebase.js varsayıldı)
...
export const fetchWithRetry = async (url, options, retries = 5, delay = 1000) => {
  try {
    ...
    // ✅ HATA DÜZELTME: HTTP durum kodu kontrolü
    if (!response.ok) {
        ...
        try {
            ...
            // JSON parse edilemezse varsayılan hata mesajı kalır
        }
...
// ==========================================
// 🟢 YARDIMCI: File -> Base64 Dönüştürücü
// ==========================================
const fileToBase64 = (file) => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
        // "data:image/jpeg;base64,..." başlığını at
...
// ==========================================
// 🟢 3. GERÇEK BACKEND ANALİZİ (GEMINI 2.5 FLASH)
// ==========================================
export const analyzePetPhoto = async (file) => {
    try {
        const functions = getFunctions(app);
        // Backend'deki 'analyzePetPhoto' fonksiyonunu çağır
        ...
        console.log("Gemini Analysis Result:", result.data);
        return result.data; // { species, breed, color, visual_details... }

    } catch (error) {
        console.error("Analysis Failed:", error);
        throw error; // Hatayı fırlat ki modal yakalasın
    }
};

// ==========================================
// 🟢 4. GERÇEK BACKEND GÖRSEL ÜRETİMİ (IMAGEN 4.0)
// ==========================================
export const generateStyledPetImage = async (file, petData, refinementPrompt = "") => {
    try {
        const functions = getFunctions(app);
        // Backend'deki 'generatePetAvatar' fonksiyonunu çağır
        const generateFn = httpsCallable(functions, 'generatePetAvatar');

        console.log("Sending Data to Imagen:", petData);

        // Backend'e tüm objeyi atıyoruz, prompt mühendisliğini backend yapıyor
        // (Veya isterseniz prompt'u burada hazırlayıp backend'e string olarak da atabilirsiniz,
        //  ancak önceki adımda backend tarafında logic kurduğumuz için objeyi atıyoruz)
        const result = await generateFn({ 
            petDetails: petData, // İsim, ırk, visual_details, gender hepsi burada
            refinement: refinementPrompt // Kullanıcının ek isteği
        });
...
export const getScanStatusSteps = (t) => [
    t('scan_step_1') || "Yüz taranıyor...",
    t('scan_step_2') || "Biyometrik veriler işleniyor...",
    t('scan_step_3') || "Irk veritabanı ile eşleştiriliyor...",
    t('scan_step_4') || "Sonuçlar hazırlanıyor..."
];
...
export const getLocalizedData = (lang, t) => {
  ...
  return {
    ...
    // 🟢 EKLENDİ: Irk ismini formatlayan fonksiyon (Hata buradaydı)
    breedName: (breedKey, type, customBreed) => {
        // Eğer özel ırk girilmişse onu döndür
        if (customBreed) return customBreed;
        
        // Eğer ırk 'Diğer' veya 'Other' ise ve customBreed yoksa
        const otherLabel = (BREEDS[type]?.[l] || [])[ (BREEDS[type]?.[l] || []).length - 1 ];
        if (breedKey === otherLabel) return otherLabel;

        // Normal ırk ismi (Çeviriye gerek yok, zaten listeden geliyor)
