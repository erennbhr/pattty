// erennbhr/pattty/pattty-df8ab6d3def020d5068a132ec11f675ce8fce13a/Yeni klasör/src/context/AppContext.jsx
...
export const AppProvider = ({ children }) => {
  ...
  // --- YENİ: AĞIRLIK BİRİMİ AYARI ---
  const [weightUnit, setWeightUnit] = useState(() => {
    // 1. Önce kayıtlı ayara bak
    ...
    // 2. Yoksa sistem diline/bölgesine bak (Otomatik Seçim)
    const userLocale = navigator.language || 'en-US';
    // ABD, Liberya ve Myanmar (Burma) dışında herkes metrik kullanır.
    // Basitlik için 'en-US' ise LBS, diğerleri KG varsayalım.
  ...
  useEffect(() => {
    ...
    if (loadedSettings) {
      ...
      // weightUnit yukarıda zaten initialize edildi, burada tekrar set etmeye gerek yok
    }
  }, []);
  ...
  // --- AYARLARI KAYDET (weightUnit dahil) ---
  useEffect(() => {
    localStorage.setItem(
      ...
      JSON.stringify({
        darkMode,
        notifications,
        weightUnit, // Yeni ayar eklendi
      })
    );
  }, [darkMode, notifications, weightUnit]);
  ...
  return (
    <AppContext.Provider 
      value={{ 
        ...
        weightUnit, setWeightUnit // Context'e eklendi
      }}
    >
      {children}
    </AppContext.Provider>
