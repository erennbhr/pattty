...
import SmartPet from './SmartPet'; // SmartPet bileşeni

// Rive Dosyaları Haritası
const RIVE_FILES = {
    // DİKKAT: Dosyanın gerçek adı 'cat_avatar.riv' ise burayı düzelt
    'cat': '/assets/cat_avatar.riv', 
    // 'dog': '/assets/dog_avatar.riv', 
};
...
const InteractiveSleepingPet = ({ type, color, className, fullSize = false }) => {
  ...
  // --- DURUM DEĞİŞTİRME MANTIĞI (ORTAK) ---
  const togglePetState = () => {
    if (timeoutRef.current) clearTimeout(timeoutRef.current);

    // Uyuyor -> Uyanık -> Kızgın
    const nextState = petState === 'sleeping' ? 'awake' : petState === 'awake' ? 'angry' : 'sleeping';
    setPetState(nextState);

    // Otomatik geri dönüş (Kızgınsa uyur, Uyanıksa bir süre sonra uyur)
  ...
  // --- TIKLAMA YÖNETİCİSİ (SVG İÇİN) ---
  const handleClick = (e) => {
    if(e) e.stopPropagation(); 
    
    // Eğer Rive dosyası varsa (Kedi), tıklamayı 'SmartPet' içinden 'onInteract' ile yönetiyoruz.
    // O yüzden buradaki genel onClick'i devre dışı bırakıyoruz.
    if (RIVE_FILES[type]) return; 

    togglePetState();
  };

  // --- ETKİLEŞİM VE SES YÖNETİCİSİ (RIVE İÇİN) ---
  const handleSmartPetInteract = (actionType) => {
    // 1. Ses Dosyasını Seç
    let soundFile = '/assets/meow.mp3'; // Varsayılan
    if (actionType === 'purr') {
        soundFile = '/assets/purr.mp3';
    }
    ...
    // 3. Durumu Değiştir
    // Sadece "Miyav" (Kısa Tıklama) yapınca uyandıralım. 
    // "Purr" (Basılı Tutma) yapınca sadece mırlasın, uykusu bölünmesin (veya sen nasıl istersen).
    if (actionType === 'meow') {
        togglePetState();
    }
  };
    ...
    if (RIVE_FILES[type]) {
        return (
            <div className="w-full h-full transform scale-125 translate-y-2">
                <SmartPet 
                    riveFile={RIVE_FILES[type]}
                    mainColor={baseColor}
                    moodId={petState === 'sleeping' ? 1 : 0}
                    eyeColor="#FCD34D"
                    className="w-full h-full"
                    onInteract={handleSmartPetInteract} // Etkileşimi buraya bağlıyoruz
                />
            </div>
        );
    }

    // === ESKİ SVG SİSTEMİ (FALLBACK) ===
