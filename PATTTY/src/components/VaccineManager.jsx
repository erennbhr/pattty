// src/components/VaccineManager.jsx
...
import { useApp } from '../context/AppContext'; // 🟢 EKLENDİ
import { getLocalizedData, generateID } from '../utils/helpers'; 
import PaywallModal from './PaywallModal'; 

const VaccineManager = ({ pet, setPets, onGoToVetLocator }) => {
  const { t, language } = useLanguage();
  const { showNotification } = useNotification(); // 🟢 DÜZELTME: Destructuring yapıldı
  const { notifyIfEnabled } = useApp(); // 🟢 EKLENDİ: Yeni bildirim fonksiyonu
  ...
  const handleAdd = () => {
    ...
    // 🟢 VACCINE kategorisi üzerinden bildirim (Kullanıcı kapatmışsa görünmez)
  ...
  const handleScanClick = async () => {
      ...
      try {
          ...
          if (Array.isArray(parsed) && parsed.length > 0) {
              ...
              // 🟢 AI kategorisi üzerinden bildirim
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom duration-500 relative pb-20">
      ...
      <div className="grid grid-cols-2 gap-4">
          <button onClick={handleScanClick} disabled={scanning} className={`relative flex flex-col items-center justify-center gap-3 p-5 rounded-[1.8rem] shadow-sm border transition-all duration-300 group overflow-hidden ${isPremium ? 'bg-white dark:bg-neutral-800 border-gray-100 dark:border-white/10 hover:border-indigo-500/30 active:scale-95' : 'bg-gray-50 dark:bg-neutral-900 border-gray-200 dark:border-white/5 opacity-90'}`}>
              ...
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${scanning ? 'bg-indigo-100 text-indigo-600 animate-spin' : isPremium ? 'bg-indigo-50 text-indigo-600 dark:bg-indigo-900/20 dark:text-indigo-400 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-indigo-500/20' : 'bg-gray-200 text-gray-400 dark:bg-white/5 grayscale'}`}>
      ...
      <div className="bg-white dark:bg-[#1A1D21] rounded-[2.5rem] p-6 shadow-sm border border-gray-100 dark:border-white/5 relative overflow-hidden">
          <div className="flex justify-between items-center mb-6 relative z-10">
              ...
              <button onClick={() => setShowForm(true)} className="w-10 h-10 bg-gray-50 dark:bg-white/5 rounded-full flex items-center justify-center text-gray-600 dark:text-gray-300 hover:bg-indigo-600 hover:text-white transition-all shadow-sm active:scale-90">
            ...
            <div className="bg-white dark:bg-[#1A1D21] w-full max-w-sm rounded-[2rem] p-6 shadow-2xl relative animate-in zoom-in-95 border border-white/10">
                ...
                <div className="space-y-4">
                    <div className="space-y-1">
                        ...
                        <select value={newVaccine.name} onChange={e => setNewVaccine(prev => ({ ...prev, name: e.target.value }))} className="w-full p-4 bg-gray-50 dark:bg-black/30 rounded-2xl font-bold text-sm outline-none dark:text-white border-2 border-transparent focus:border-indigo-500 focus:bg-white dark:focus:bg-black transition-all appearance-none">
                    ...
                    <div className="space-y-1">
                        ...
                        <input type="date" value={newVaccine.date} onChange={e => setNewVaccine(prev => ({ ...prev, date: e.target.value }))} className="w-full p-4 bg-gray-50 dark:bg-black/30 rounded-2xl font-bold text-sm outline-none dark:text-white border-2 border-transparent focus:border-indigo-500 focus:bg-white dark:focus:bg-black transition-all min-h-[56px]" />
                    </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-md px-6 pt-safe-top pb-safe-bottom">
          <div className="w-full max-w-md bg-white dark:bg-[#1A1D21] rounded-[2rem] shadow-2xl p-6 relative border border-white/10 animate-in zoom-in-95">
            ...
            <div className="flex gap-3">
              <button onClick={() => { setShowAiResultModal(false); setAiSuggestedVaccines([]); }} className="flex-1 py-3.5 rounded-2xl text-sm font-bold bg-gray-100 dark:bg-white/10 text-gray-800 dark:text-white hover:bg-gray-200 dark:hover:bg-white/20 transition-colors">{t('ok_btn')}</button>
