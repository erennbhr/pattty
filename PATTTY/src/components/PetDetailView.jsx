...
const PetDetailView = ({ pet, setPets, onBack }) => {
    ...
    return (
        <div className="fixed inset-0 z-[60] flex flex-col bg-gray-50 dark:bg-black animate-in slide-in-from-right duration-300">
            {/* DÜZELTME YAPILDI:
                1. pt-safe-top: Çentik payını koruduk.
                2. mt-2 KALDIRILDI: "Çok boşluk var" sorununu çözmek için margin silindi.
                3. pb-3: Header içeriğinin altına biraz nefes payı eklendi.
            */}
            <div className="pt-safe-top px-4 pb-3 flex items-center justify-between bg-white dark:bg-neutral-900 border-b border-gray-100 dark:border-neutral-800 shrink-0 shadow-sm relative z-50">
                ...
                {/* Başlık Dinamik Çeviri - Ortalanmış ve truncate edilmiş */}
                <h2 className="font-bold text-lg dark:text-white absolute left-1/2 -translate-x-1/2 max-w-[60%] truncate text-center">
                    {subTab !== 'summary' ? t(`tab_${subTab}`) : pet.name}
                </h2>
                {/* Sağ tarafı dengelemek için boş div (Geri butonu kadar yer kaplar) */}
                <div className="w-10"/> 
            </div>
