...
import { useApp } from '../context/AppContext'; // YENİ: Context eklendi
...
const PetDetailView = ({ pet, setPets, onBack }) => {
    const { t } = useLanguage();
    const { weightUnit } = useApp(); // YENİ: Global birim ayarı
    ...
    // YENİ: Kilo Gösterimi Hesaplama
    // Veritabanındaki ham veri her zaman KG'dir.
    const rawWeight = pet.weights?.[pet.weights.length-1]?.weight || pet.weight || 0;
    
    // Eğer birim LBS ise dönüşüm yap, yoksa olduğu gibi al
    const displayWeight = weightUnit === 'kg' ? rawWeight : rawWeight * 2.20462;
    
    // Birim etiketi (KG veya LBS)
