...
const AddPetModal = ({ onClose, onAdd, initialData }) => {
  ...
  // 🟢 HATA DÜZELTME: GÜVENLİ BİLDİRİM ERİŞİMİ
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="w-full max-w-lg bg-white/95 dark:bg-[#111417]/95 border border-white/20 dark:border-white/10 rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh] transition-all duration-500 animate-in zoom-in-95 slide-in-from-bottom-8">
        
        {/* Header */}
        <div className="p-6 flex items-center justify-between border-b border-gray-100 dark:border-white/5 bg-white/50 dark:bg-white/5 backdrop-blur-xl sticky top-0 z-20">
            <div>
        ...
        <div className="flex-1 overflow-y-auto p-6 scroll-smooth custom-scrollbar">
            ...
            {step === 1 && !initialData && (
                <div className="flex flex-col h-full justify-center gap-8 animate-in slide-in-from-right duration-500">
                    <div 
                        ...
                        className="flex-1 min-h-[320px] border-2 border-dashed border-indigo-200 dark:border-indigo-500/30 rounded-[2.5rem] bg-indigo-50/30 dark:bg-indigo-500/5 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-all cursor-pointer flex flex-col items-center justify-center group relative overflow-hidden active:scale-[0.99]"
                    >
                        ...
                        {previewUrl ? (
                            <>
                                ...
                                <div className="z-10 bg-white/90 dark:bg-black/80 backdrop-blur-xl px-6 py-3 rounded-2xl text-gray-900 dark:text-white font-bold flex items-center gap-3 shadow-2xl border border-white/20 transform group-hover:-translate-y-1 transition-transform">
                                    <Edit3 size={18} className="text-indigo-500"/> {t('change_photo')} 
                                </div>
                            </>
                        ) : (
                            <div className="text-center p-6">
                                <div className="w-28 h-28 bg-white dark:bg-white/5 rounded-full flex items-center justify-center text-indigo-500 dark:text-indigo-400 mb-6 group-hover:scale-110 transition-transform duration-500 shadow-xl shadow-indigo-500/10">
            ...
            {step === 2 && (
                <div className="flex flex-col items-center justify-center h-full py-8 animate-in zoom-in duration-700 relative">
                    <div className="relative w-72 h-72 rounded-[2.5rem] overflow-hidden border-4 border-indigo-500/30 dark:border-indigo-500/20 shadow-[0_0_100px_-20px_rgba(99,102,241,0.5)] bg-black">
                        ...
                        <input 
                            ...
                            className={`w-full p-5 rounded-2xl font-bold text-lg outline-none transition-all appearance-none placeholder:font-medium
                                ${errors.name 
                                    ...
                                    : 'bg-gray-50 dark:bg-white/5 border-2 border-transparent focus:border-indigo-500 focus:bg-white dark:focus:bg-black text-gray-900 dark:text-white shadow-sm'
                                }`}
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-4 gap-3">
                            ...
                            <button 
                                ...
                                className={`flex flex-col items-center gap-3 p-4 rounded-2xl border transition-all duration-300 ${showSecondaryTypes || formData.type === 'other' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-300' : 'border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-400'}`}
                            >
                                {showSecondaryTypes ? <ChevronDown size={32} /> : <MoreHorizontal size={32} />}
                                <span className="text-xs font-bold tracking-wide">{t('other_types')}</span>
                            </button>
                        </div>
                        
                        {(showSecondaryTypes || formData.type === 'other') && (
                            <div className="grid grid-cols-4 gap-3 mt-3 animate-in slide-in-from-top-4 fade-in duration-300">
                                ...
                                <input 
                                    ...
                                    className={`w-full p-5 rounded-2xl font-bold outline-none transition-all appearance-none
                                        ${errors.customType
                                            ...
                                            : 'bg-indigo-50 dark:bg-indigo-500/10 border-2 border-indigo-100 dark:border-indigo-500/30 text-indigo-900 dark:text-indigo-100 focus:border-indigo-500'
                                        }`}
                                />
                            </div>
                        )}
                                ...
                                <div className="mt-3 animate-in slide-in-from-top-2">
                                    <input 
                                        ...
                                        className={`w-full p-4 rounded-xl font-bold outline-none transition-all text-sm
                                            ${errors.customBreed
                                                ? 'bg-red-50 dark:bg-red-900/20 border-2 border-red-500'
                                                : 'bg-indigo-50 dark:bg-indigo-500/10 border-2 border-indigo-100 dark:border-indigo-500/30'
                                            }`}
                                    />
                                </div>
                            )}
                    ...
                    <div>
                        ...
                        <div className={`flex gap-4 p-1.5 rounded-2xl transition-all ${errors.gender ? 'bg-red-50 dark:bg-red-900/10 ring-2 ring-red-500' : ''}`}>
                            <button 
                                ...
                                className={`flex-1 py-5 rounded-2xl border-2 flex items-center justify-center gap-3 font-bold text-lg transition-all duration-300 active:scale-95 ${formData.gender === 'female' ? 'border-pink-500 bg-pink-50 dark:bg-pink-500/10 text-pink-500 shadow-lg shadow-pink-500/20' : 'border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-400 hover:bg-gray-50'}`}
                            >
                                <Venus size={24} /> {t('gender_f_label')}
                            </button>
                            <button 
                                onClick={() => setFormData({...formData, gender: 'male'})}
                                className={`flex-1 py-5 rounded-2xl border-2 flex items-center justify-center gap-3 font-bold text-lg transition-all duration-300 active:scale-95 ${formData.gender === 'male' ? 'border-blue-500 bg-blue-50 dark:bg-blue-500/10 text-blue-500 shadow-lg shadow-blue-500/20' : 'border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-400 hover:bg-gray-50'}`}
                            >
                    ...
                    {['cat', 'dog', 'rabbit'].includes(formData.type) && (
                        <div className="animate-in slide-in-from-right pt-2">
                            ...
                            <div className={`relative w-full h-16 bg-gray-100 dark:bg-white/5 rounded-2xl p-1.5 flex items-center cursor-pointer transition-all ${errors.isNeutered ? 'ring-2 ring-red-500 bg-red-50 dark:bg-red-900/10' : ''}`}>
                                <div 
                                    className={`absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-white dark:bg-[#23262B] rounded-xl shadow-lg transition-all duration-500 ease-spring ${formData.isNeutered ? 'left-[calc(50%+3px)]' : 'left-1.5'} ${formData.isNeutered === null ? 'opacity-0' : 'opacity-100'}`}
                                />
        ...
        {step !== 2 && (
            <div className="p-6 border-t border-gray-100 dark:border-white/5 bg-white/80 dark:bg-[#111417]/80 backdrop-blur-xl flex gap-4 sticky bottom-0 z-20">
                {step > 1 && step < 5 && (
                    <button 
                        onClick={handleBack}
        ...
        {showRegenInput && (
            <div className="fixed inset-0 z-[70] bg-black/80 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300">
                <div className="bg-white dark:bg-[#1A1D21] border border-gray-200 dark:border-white/10 w-full max-w-sm rounded-[2rem] p-6 shadow-2xl relative animate-in zoom-in-95 duration-300">
                    ...
                    <textarea 
                        ...
                        className="w-full bg-gray-50 dark:bg-black/20 border-2 border-gray-100 dark:border-white/10 rounded-2xl p-4 text-sm font-medium text-gray-900 dark:text-white outline-none focus:border-indigo-500 mb-6 resize-none h-32 transition-colors placeholder:text-gray-400"
                    />
