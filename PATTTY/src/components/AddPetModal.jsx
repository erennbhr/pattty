...
const AddPetModal = ({ onClose, onAdd, initialData }) => {
  ...
  // Modal açılışında kilo verilerini doldur
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center sm:p-4">
      ...
      {/* Modal İçeriği */}
      <div className="bg-white dark:bg-[#111417] w-full max-w-lg sm:rounded-3xl rounded-none relative pointer-events-auto animate-in slide-in-from-bottom duration-300 shadow-2xl flex flex-col h-full sm:h-auto sm:max-h-[90vh]">
          
          {/* HEADER: pt-safe-top eklendi, bg-opacity kaldırıldı, tam opak yapıldı */}
          <div className="p-4 border-b border-gray-100 dark:border-white/5 flex justify-between items-center bg-white dark:bg-[#111417] sm:rounded-t-3xl sticky top-0 z-20 pt-safe-top">
               <h2 className="font-bold text-lg text-gray-900 dark:text-white flex items-center gap-2">
                 {initialData ? <Edit3 size={18} className="text-indigo-500"/> : <Plus size={18} className="text-indigo-500"/>} 
                 {initialData ? t('update') : t('form_grow')}
               </h2>
              <button onClick={onClose} className="p-2 bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-gray-300 rounded-full hover:bg-gray-200 dark:hover:bg-white/20 transition-colors">
                <X size={18}/>
              </button>
          </div>

          {/* İçerik */}
          <div className="shrink-0 flex justify-center py-6 bg-gray-50 dark:bg-black/20 border-b border-gray-100 dark:border-white/5 relative">
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-6 pt-4 space-y-6">
              ...
              <div className="space-y-3">
                  ...
                  {!showOther ? (
                      <button onClick={() => setShowOther(true)} className="w-full p-3 rounded-2xl border-2 border-dashed border-gray-200 dark:border-white/10 text-gray-400 font-bold text-sm flex items-center justify-center gap-2 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div className="space-y-2">
                      ...
                      <input name="name" value={formData.name} onChange={handleChange} placeholder={t('name_placeholder')} className={`w-full p-4 bg-gray-50 dark:bg-[#1A1D21] rounded-2xl outline-none border-2 focus:border-indigo-500/50 dark:text-white transition-all font-bold ${errors.name ? 'border-red-500 bg-red-50 dark:bg-red-900/10' : 'border-transparent dark:border-white/5'}`} />
                  </div>
                  <div className="space-y-2">
                        ...
                        {formData.type === 'other' ? (
                            <input name="breed" value={formData.breed} onChange={handleChange} placeholder={t('custom_type_placeholder')} className={`w-full p-4 bg-gray-50 dark:bg-[#1A1D21] rounded-2xl outline-none border-2 focus:border-indigo-500/50 dark:text-white font-bold transition-all ${errors.breed ? 'border-red-500 bg-red-50 dark:bg-red-900/10' : 'border-transparent dark:border-white/5'}`} />
                        ) : (
                             <div className="relative">
                                <select name="breed" value={formData.breed} onChange={handleChange} className={`w-full p-4 bg-gray-50 dark:bg-[#1A1D21] rounded-2xl outline-none border-2 focus:border-indigo-500/50 dark:text-white font-bold appearance-none transition-all ${errors.breed ? 'border-red-500 bg-red-50 dark:bg-red-900/10' : 'border-transparent dark:border-white/5'}`}>
              ...
              <div className="grid grid-cols-1 gap-5">
                    <div className="space-y-2">
                      ...
                      <div className={`flex bg-gray-100 dark:bg-[#1A1D21] p-1.5 rounded-2xl border-2 transition-all ${errors.gender ? 'border-red-500 ring-1 ring-red-500 bg-red-50 dark:bg-red-900/10' : 'border-transparent dark:border-white/5'}`}>
                          <button onClick={() => {setFormData({...formData, gender: 'female'}); if(errors.gender) setErrors({...errors, gender:false})}} className={`flex-1 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${formData.gender === 'female' ? 'bg-white dark:bg-[#23262B] shadow-sm text-pink-500' : 'text-gray-400'}`}><Venus size={16}/> {t('gender_f_label')}</button>
                          <button onClick={() => {setFormData({...formData, gender: 'male'}); if(errors.gender) setErrors({...errors, gender:false})}} className={`flex-1 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${formData.gender === 'male' ? 'bg-white dark:bg-[#23262B] shadow-sm text-blue-500' : 'text-gray-400'}`}><Mars size={16}/> {t('gender_m_label')}</button>
                      </div>
                    </div>

                    <div className="space-y-3">
                      <label className="text-xs font-bold text-gray-400 tracking-wider block">{toUpper(t('form_color'))}</label>
                      <div className="flex flex-wrap justify-start gap-4 p-2 bg-gray-50 dark:bg-[#1A1D21] rounded-2xl border border-gray-100 dark:border-white/5">
                          {colorPresets.map(color => (
                              <button 
                                key={color} 
                                onClick={() => setFormData({...formData, color})}
                                className={`w-10 h-10 rounded-full border-2 transition-all duration-300 transform 
                                    ${formData.color === color ? 'border-indigo-500 scale-125 ring-2 ring-indigo-200 dark:ring-indigo-900 shadow-lg' : 'border-gray-200 dark:border-white/10 hover:scale-110'}
                                `}
                          ...
                          <div className="relative group">
                              <div className={`w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-500 via-purple-500 to-pink-500 border-2 transition-all duration-300 flex items-center justify-center
                                ${!colorPresets.includes(formData.color) ? 'border-indigo-500 scale-125 ring-2 ring-indigo-200 dark:ring-indigo-900 shadow-lg' : 'border-gray-200 dark:border-white/10 hover:scale-110'}
                              `}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div className="space-y-2">
                       ...
                       <input type="date" name="birthDate" value={formData.birthDate} onChange={handleChange} className={`w-full p-4 bg-gray-50 dark:bg-[#1A1D21] rounded-2xl outline-none border-2 focus:border-indigo-500/50 dark:text-white font-bold text-sm transition-all min-h-[56px] block ${errors.birthDate ? 'border-red-500 bg-red-50 dark:bg-red-900/10' : 'border-transparent dark:border-white/5'}`} />
                  </div>
                  <div className="space-y-2">
                       <label className="text-xs font-bold text-gray-400 tracking-wider block">{toUpper(t('form_weight'))}</label>
                       
                       <div className={`flex gap-2 p-1.5 bg-gray-50 dark:bg-[#1A1D21] rounded-2xl border-2 transition-all ${errors.weight ? 'border-red-500 bg-red-50 dark:bg-red-900/10' : 'border-transparent dark:border-white/5'}`}>
                            <div className="relative flex-1">
                                <input 
                                    ...
                                    className="w-full p-2.5 bg-white dark:bg-[#23262B] rounded-xl text-center dark:text-white font-bold outline-none shadow-sm" 
                                />
          ...
          <div className="p-6 bg-white dark:bg-[#111417] border-t border-gray-100 dark:border-white/5 rounded-b-3xl sticky bottom-0 z-20">
