...
import {
  ...
  Edit2, Loader2,
  Calendar, Syringe, RefreshCcw, Zap // 🟢 YENİ İKONLAR EKLENDİ
} from 'lucide-react';
...
const AccountSettings = () => {
  ...
  // 🟢 notifications ve notifyIfEnabled EKLENDİ
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // 🟢 UPDATES kategorisi üzerinden bildirim
  ...
  /* --------------------------------------------------------
     NOTIFICATION SETTINGS - 🟢 GÜNCELLENDİ
   -------------------------------------------------------- */
  if (currentView === 'notifications') {
    const ToggleButton = ({ active, onClick }) => (
        <button 
            onClick={(e) => { e.stopPropagation(); onClick(); }} 
            className={`w-12 h-7 rounded-full p-1 transition-colors duration-300 ${active ? 'bg-indigo-600' : 'bg-gray-300 dark:bg-gray-600'}`}
        >
