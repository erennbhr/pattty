...
import {
  ...
  Phone, MapPin, Info, Trash2, Shield, Scale // Scale ikonu eklendi
} from 'lucide-react';
...
const AccountSettings = () => {
  ...
  // weightUnit ve setWeightUnit eklendi
  const { darkMode, setDarkMode, notifications, setNotifications, weightUnit, setWeightUnit } = useApp();
  ...
  /* --------------------------------------------------------
      UNIT SELECTION VIEW (YENİ EKLENDİ)
  -------------------------------------------------------- */
  if (currentView === 'settings_unit') {
    return (
      <SubSettingsLayout title={t('set_unit')} onBack={() => setCurrentView('settings')}>
        <div className="space-y-2">
            <SettingsItem
              icon={Check}
              label={t('unit_kg')}
