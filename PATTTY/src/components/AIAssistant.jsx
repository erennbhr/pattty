...
import { generateID } from '../utils/helpers'; // API Key importunu kaldırdık
...
const AIAssistant = () => {
    ...
    const handleSend = async () => {
        // --- 1. Premium ve Limit Kontrolleri ---
        ...
        recordMessage(); // Kullanım hakkından düş

        // --- 2. Mesajı Ekrana Bas (Kullanıcı Beklemesin) ---
        ...
        // Resmi geçici hafızaya al ve temizle
        ...
        try {
            // --- 3. GÜVENLİ SUNUCU İÇİN VERİ HAZIRLA ---
            
            // Kullanıcı mesajı (Yazı + Opsiyonel Resim)
            const userPart = [];
            if (userText) userPart.push({ text: userText });
            if (currentAttachment) {
                // Base64 formatındaki resmi temizle (data:image/png;base64, kısımlarını at)
            ...
            // 🔴 BURAYA DİKKAT: Terminalden aldığın URL'yi buraya ekledik!
            const SERVER_URL = "https://us-central1-pattty-7adff.cloudfunctions.net/chatWithAI";

            // --- 4. Sunucuya İstek At (Fetch) ---
            ...
            let aiText = data.text || "Cevap alınamadı.";
            
            // --- 5. Cevabı İşle (Aksiyonlar ve Ekrana Yazma) ---
            ...
            setMessages(prev => [...prev, { role: 'model', text: "⚠️ Bağlantı hatası veya sunucu yoğunluğu." }]);
            setLoading(false);
        }
    };
