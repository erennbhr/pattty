...
const AIAssistant = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-gray-50 dark:bg-black relative">
            ...
            <div className="flex items-center justify-between p-4 bg-white/80 dark:bg-white/5 dark:backdrop-blur-xl border-b border-gray-100 dark:border-white/5 sticky top-0 z-20">
                ...
                <button onClick={() => { setMessages([{ role: 'model', text: t('ai_intro') }]); setAiMemory([]); }} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-white/5 rounded-xl transition-all">
                    <Trash2 size={18} />
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-32">
                {messages.slice(1).map((m, i) => ( 
                    <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        {/* 🟢 RENKLİ BALONCUKLAR KORUNDU */}
                        <div className={`max-w-[85%] p-3.5 rounded-2xl text-sm leading-relaxed shadow-sm relative group flex flex-col gap-2 ${m.role === 'user' ? 'bg-indigo-600 text-white rounded-br-sm' : 'bg-white dark:bg-white/5 dark:backdrop-blur-sm dark:text-gray-200 border border-gray-100 dark:border-white/5 rounded-bl-sm'}`}>
                ...
                {streamingContent && (
                    <div className="flex justify-start">
                        <div className="max-w-[85%] p-4 rounded-2xl rounded-bl-sm shadow-sm border border-gray-100 dark:border-white/5 bg-white dark:bg-white/5 dark:backdrop-blur-sm dark:text-gray-200 text-sm leading-relaxed">
                ...
                {loading && !streamingContent && (
                    <div className="flex justify-start">
                        <div className="bg-white dark:bg-white/5 px-4 py-3 rounded-2xl rounded-bl-sm shadow-sm border border-gray-100 dark:border-white/5 flex gap-1 items-center">
            ...
            <div className="fixed bottom-20 left-0 right-0 p-4 bg-gray-50/90 dark:bg-black/90 backdrop-blur-md border-t border-gray-100 dark:border-white/5 z-30">
                ...
                {isBlocked && (
                    <div className="absolute inset-0 bg-white/80 dark:bg-black/80 backdrop-blur-sm z-40 flex flex-col items-center justify-center p-4 text-center">
                        <div className="bg-white dark:bg-[#1C1C1E] p-4 rounded-3xl shadow-xl border border-gray-100 dark:border-white/5 w-full max-w-xs animate-in slide-in-from-bottom">
                ...
                {attachment && (
                    <div className="mb-2 flex items-center gap-2 bg-white dark:bg-white/5 p-2 rounded-xl w-fit shadow-sm border border-gray-100 dark:border-white/5 animate-in slide-in-from-bottom">
                        <img src={attachment.preview} alt={t('image_preview_alt')} className="w-12 h-12 rounded-lg object-cover" />
                        <button onClick={clearAttachment} className="p-1 bg-gray-100 dark:bg-white/10 rounded-full hover:bg-red-500 hover:text-white transition-colors">
                            <X size={14} />
                        </button>
                    </div>
                )}

                <div className="flex gap-2 items-end bg-white dark:bg-white/5 p-2 rounded-[1.5rem] shadow-sm border border-gray-200 dark:border-white/5 focus-within:border-indigo-500/50 focus-within:ring-2 focus-within:ring-indigo-500/10 transition-all">
                    ...
                    <button 
                        ...
                        className="bg-indigo-600 text-white p-3 rounded-full disabled:opacity-50 hover:scale-105 active:scale-95 transition-all shadow-lg shadow-indigo-200 dark:shadow-none"
                    >
                        <ArrowRight size={20} />
                    </button>
                </div>
